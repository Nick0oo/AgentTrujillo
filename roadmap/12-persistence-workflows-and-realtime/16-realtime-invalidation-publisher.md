---
id: AT-12-16
title: Publish minimal invalidations transactionally
...
touches:
  create:
    - src/realtime/invalidation-publisher.ts
    - tests/realtime/invalidation-publisher.test.ts
  modify: []
  test:
    - tests/realtime/invalidation-publisher.test.ts
exclusive_paths:
...
commit:
  message: "feat(realtime): publish minimal invalidations transactionally"
---

## Outcome

A typed publisher appends one minimal invalidation in the same transaction as a committed projection/domain change and supports idempotent replay without directly broadcasting PHI.
...
AT-12-15 plus installed Workflow DevKit documentation and the current projection/outbox/Realtime schema.
...
A typed publisher appends one minimal invalidation in the same transaction as a committed projection/domain change and supports idempotent replay without directly broadcasting PHI. Exact input/result schemas, step/effect boundaries, replay keys, failure classification, authorization, privacy, tests, and evidence are included.
...
Export InvalidationResource registry and publishInvalidation(tx,scope,{resourceId,version,sourceEventId}); every resource kind has an authorized refetch endpoint.

## Technical design

Validate composite scope and monotonic version, insert through server-only function in caller transaction, dedupe source event, never publish before commit, emit safe opaque telemetry, and cap rate/coalesce only when version semantics preserve reconciliation.
...
Realtime is cache invalidation, not clinical event/alert. Urgent safety never uses it; medication reminders use their own opted-in workflow.
...
Cover rollback, concurrent versions, duplicate source, out-of-order update, unknown resource, scope mismatch, publication outage and retry; also serialization, duplicate/concurrent run, crash before/after effect, cancellation, revocation, deterministic result and zero cross-scope output.
...
Commit exclusive paths with feat(realtime): publish minimal invalidations transactionally; no production schedule, remote migration, deployment, or unrelated edit.
