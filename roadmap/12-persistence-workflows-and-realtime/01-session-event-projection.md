---
id: AT-12-01
title: Project Eve session lifecycle
module: 12-persistence-workflows-and-realtime
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-11-17, AT-02-04]
blocks: [AT-12-02, AT-12-05]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/projections/session-event-projection.ts
    - tests/projections/session-event-projection.test.ts
  modify: []
  test:
    - tests/projections/session-event-projection.test.ts
exclusive_paths:
...
commit:
  message: "feat(persistence): project eve session lifecycle"
---

## Outcome

An idempotent projector maps allowed Eve session lifecycle events into the owned Supabase session record and safe channel-event outbox without making Eve IDs an authorization source.
...
AT-11-17, AT-02-04 plus installed Workflow DevKit documentation and the current projection/outbox/Realtime schema.
...
An idempotent projector maps allowed Eve session lifecycle events into the owned Supabase session record and safe channel-event outbox without making Eve IDs an authorization source. Exact input/result schemas, step/effect boundaries, replay keys, failure classification, authorization, privacy, tests, and evidence are included.
...
Export projectSessionEvent(tx,envelope), SessionProjectionEvent union and outcome; envelope carries trusted composite scope, source event ID/version/sequence and safe metadata.

## Technical design

Validate source schema/order, reauthorize/bind existing ownership, monotonic state transition, upsert by source event identity, append safe channel event in same transaction, and quarantine unknown/out-of-order events.
...
Projection never stores prompts/reasoning/raw content or creates clinical meaning. Emergency terminal state cannot reopen into model/tool activity.
...
Cover duplicate/out-of-order/replayed source, unknown state, cross-scope session, terminal transition, partial transaction, deleted owner and schema drift; also serialization, duplicate/concurrent run, crash before/after effect, cancellation, revocation, deterministic result and zero cross-scope output.
...
Commit exclusive paths with feat(persistence): project eve session lifecycle; no production schedule, remote migration, deployment, or unrelated edit.
