---
id: AT-12-11
title: Execute retention and deletion cleanup
module: 12-persistence-workflows-and-realtime
status: pending
execution: parallel
parallel_group: durable-workflows
depends_on: [AT-12-05, AT-09-10]
blocks: [AT-12-13, AT-12-18]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/workflows/retention-cleanup.ts
    - tests/workflows/retention-cleanup.test.ts
  modify: []
  test:
    - tests/workflows/retention-cleanup.test.ts
exclusive_paths:
...
commit:
  message: "feat(workflow): execute retention and deletion cleanup"
---

## Outcome

A durable cleanup workflow removes or tombstones all policy-targeted primary and derived artifacts, verifies closure, and produces privacy-safe deletion evidence without resurrecting data on replay.
...
AT-12-05, AT-09-10 plus installed Workflow DevKit documentation and the current projection/outbox/Realtime schema.
...
A durable cleanup workflow removes or tombstones all policy-targeted primary and derived artifacts, verifies closure, and produces privacy-safe deletion evidence without resurrecting data on replay. Exact input/result schemas, step/effect boundaries, replay keys, failure classification, authorization, privacy, tests, and evidence are included.
...
Export retentionCleanupWorkflow(request) and use-step functions loadAuthorizedRequest, revokeEligibility, deleteVectors, deleteObjects, deleteRows, purgeCaches, verifyClosure, finalizeEvidence.

## Technical design

Immediate eligibility revocation is first idempotent step; deterministic deletion plan enumerates memory/doc/message/vector/link/cache/summary artifacts by composite scope; each step is retry-safe; legal hold is verified; final orphan query must be empty before completion.
...
Deletion audit retains only approved opaque proof, never content. Workflow cannot bypass legal authority or delete another child/care space.
...
Cover repeat request, partial step crash, object unavailable, indexer race, legal hold, new derived artifact, scope mismatch, provider outage and closure failure; also serialization, duplicate/concurrent run, crash before/after effect, cancellation, revocation, deterministic result and zero cross-scope output.
...
Commit exclusive paths with feat(workflow): execute retention and deletion cleanup; no production schedule, remote migration, deployment, or unrelated edit.
