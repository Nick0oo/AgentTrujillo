---
id: AT-08-08
title: Apply nutrition abstention policy
module: 08-nutrition-and-development
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-08-05, AT-08-07, AT-04-10]
blocks: [AT-08-17, AT-10-10]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/clinical/nutrition/abstention.ts
    - tests/clinical/nutrition/abstention.test.ts
  modify: []
  test:
    - tests/clinical/nutrition/abstention.test.ts
exclusive_paths:
...
commit:
  message: "feat(nutrition): apply nutrition abstention policy"
---

## Outcome

A final policy suppresses generic output when clinical complexity, reaction, swallowing, growth, age, source, or uncertainty exceeds scope.
...
AT-08-05, AT-08-07, AT-04-10 and the approved sources, licenses, and contracts named below.
...
A final policy suppresses generic output when clinical complexity, reaction, swallowing, growth, age, source, or uncertainty exceeds scope. Exact schemas, provenance, deterministic behavior, fail-closed states, tests, and evidence are included.
...
Export applyNutritionAbstention with educational_guidance, insufficient_data, professional_review, or emergency_only and approved reasons.

## Technical design

Apply emergency result first, then exclusions, missing data, package validity, and content safety; only approved boundary/result passes.
...
No diagnosis, therapeutic diet, allergy determination, booking/contact/handoff, or urgent extras.
...
Cover urgent reaction, allergy, swallowing, malnutrition context, unavailable package, conflicting facts and precedence; also fixed cutoff, stable digest/order, revocation, scope mismatch, and atomicity where applicable.
...
Commit exclusive paths with feat(nutrition): apply nutrition abstention policy; no remote mutation, activation, or unrelated edit.
