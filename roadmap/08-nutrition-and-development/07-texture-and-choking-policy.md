---
id: AT-08-07
title: Enforce texture and choking policy
module: 08-nutrition-and-development
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-08-05, AT-08-06]
blocks: [AT-08-08]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/clinical/nutrition/texture-policy.ts
    - tests/clinical/nutrition/texture-policy.test.ts
  modify: []
  test:
    - tests/clinical/nutrition/texture-policy.test.ts
exclusive_paths:
...
commit:
  message: "feat(nutrition): enforce texture and choking policy"
---

## Outcome

A deterministic policy filters food form, shape, size, texture, supervision statements, and approach using approved rules.
...
AT-08-05, AT-08-06 and the approved sources, licenses, and contracts named below.
...
A deterministic policy filters food form, shape, size, texture, supervision statements, and approach using approved rules. Exact schemas, provenance, deterministic behavior, fail-closed states, tests, and evidence are included.
...
Export evaluateTextureSafety and TextureDisposition with allowed, transform-required, excluded, or review-required plus approved text.

## Technical design

Resolve each ingredient/preparation form; transformations are approved content IDs. Unknown swallowing ability or risky forms fail closed.
...
Do not claim zero choking risk, provide first aid, or let BLW bypass preparation/supervision; active choking is urgent preflight.
...
Cover grapes, hard foods, nuts/butters, bones, sausages, sticky foods, readiness bounds, unknown form and BLW attacks; also fixed cutoff, stable digest/order, revocation, scope mismatch, and atomicity where applicable.
...
Commit exclusive paths with feat(nutrition): enforce texture and choking policy; no remote mutation, activation, or unrelated edit.
