---
id: AT-08-11
title: Query caregiver-safe milestones
...
touches:
  create:
    - src/application/development/milestone-query.ts
    - tests/application/development/milestone-query.test.ts
  modify: []
  test:
    - tests/application/development/milestone-query.test.ts
exclusive_paths:
...
commit:
  message: "feat(development): query caregiver-safe milestones"
---

## Outcome

A minimal query returns approved age-window communication items without scoring or comparing the child to pass thresholds.
...
AT-08-10 and the approved sources, licenses, and contracts named below.
...
A minimal query returns approved age-window communication items without scoring or comparing the child to pass thresholds. Exact schemas, provenance, deterministic behavior, fail-closed states, tests, and evidence are included.
...
Export MilestoneQuery.execute(scope,{frameworkId,ageWindow,locale,cutoff}) returning ordered text/citations/disclaimer.

## Technical design

Trusted age selects released caregiver items; no completed/missed totals or automated concern classification.
...
Never label missing milestones delay, normal, abnormal, passed, or failed; concerns recommend a pediatrician only.
...
Cover age boundaries, corrected-age policy, empty package, professional leakage, locale fallback, revocation and order; also fixed cutoff, stable digest/order, revocation, scope mismatch, and atomicity where applicable.
...
Commit exclusive paths with feat(development): query caregiver-safe milestones; no remote mutation, activation, or unrelated edit.
