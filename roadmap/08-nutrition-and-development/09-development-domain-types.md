...
touches:
  create:
    - src/clinical/development/types.ts
    - tests/clinical/development/types.test.ts
  modify: []
  test:
    - tests/clinical/development/types.test.ts
exclusive_paths:
...
commit:
  message: "feat(development): define development diary contracts"
---

## Outcome

Closed contracts separate caregiver observations, communication milestones, professional instruments, attachments, and concerns.
...
Closed contracts separate caregiver observations, communication milestones, professional instruments, attachments, and concerns. Exact schemas, provenance, deterministic behavior, fail-closed states, tests, and evidence are included.
...
Export DevelopmentObservationDraft, CaregiverMilestoneItem, DevelopmentFramework, DevelopmentConcern, and AttachmentReference.

## Technical design

Mark audience caregiver or professional; professional scoring/item/cutoff fields are impossible on consumer/model surfaces.
...
No screening score, developmental-age estimate, diagnosis, delay/autism inference, or assurance is representable.
...
Cover authority/professional fields, invalid ages/domains/status, observation/inference separation and serialization; also fixed cutoff, stable digest/order, revocation, scope mismatch, and atomicity where applicable.
...
Commit exclusive paths with feat(development): define development diary contracts; no remote mutation, activation, or unrelated edit.
