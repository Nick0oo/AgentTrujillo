---
id: AT-09-17
title: Issue short-lived private download tickets
module: 09-clinical-memory-and-documents
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-09-15, AT-09-16]
blocks: [AT-09-19]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/application/documents/download-ticket-service.ts
    - tests/application/documents/download-ticket-service.test.ts
  modify: []
  test:
    - tests/application/documents/download-ticket-service.test.ts
exclusive_paths:
...
commit:
  message: "feat(documents): issue short-lived private download tickets"
---

## Outcome

An authorized service issues a minimal-TTL signed read URL for one permitted malware-clean private object after current resource/scope authorization.
...
AT-09-15, AT-09-16 plus the current schema, provider capability, and policies named below.
...
An authorized service issues a minimal-TTL signed read URL for one permitted malware-clean private object after current resource/scope authorization. Exact schemas, algorithms, authorization, provenance, replay/lifecycle behavior, tests, and evidence are included.
...
Export PrivateDownloadTicketService.issue(scope,{documentId,purpose}) with URL, expiry, content disposition/name and audit ID; URL never enters model/tool-safe output.

## Technical design

Reauthorize document and linked-resource visibility at issuance; require clean/active/not-deleted state; create signed URL for exact object; store ticket digest/metadata only. TTL accounts for signed URLs remaining valid after auth revocation; no refresh token.
...
No public URL/list access, permanent link, model exposure, or download of unscanned/quarantined content.
...
Cover revoked guardian, link visibility change, expired/deleted/quarantined doc, provider error, URL log leak, repeated issue limits, cross-child ID and auth rotation semantics; also fixed cutoff, stable ordering/digest, revocation, concurrent retry, and atomic rollback where applicable.
...
Commit exclusive paths with feat(documents): issue short-lived private download tickets; no remote mutation, deployment, activation, or unrelated edit.
