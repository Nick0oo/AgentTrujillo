---
id: AT-09-18
title: Keep document extraction untrusted and draft
module: 09-clinical-memory-and-documents
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-09-15, AT-09-16, AT-09-03]
blocks: [AT-09-19, AT-12-10]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/clinical/documents/extraction-policy.ts
    - tests/clinical/documents/extraction-policy.test.ts
  modify: []
  test:
    - tests/clinical/documents/extraction-policy.test.ts
exclusive_paths:
...
commit:
  message: "feat(documents): keep document extraction untrusted and draft"
---

## Outcome

A deterministic policy converts verified processor output into bounded typed draft candidates while preventing autonomous clinical writes or model authority.
...
AT-09-15, AT-09-16, AT-09-03 plus the current schema, provider capability, and policies named below.
...
A deterministic policy converts verified processor output into bounded typed draft candidates while preventing autonomous clinical writes or model authority. Exact schemas, algorithms, authorization, provenance, replay/lifecycle behavior, tests, and evidence are included.
...
Export evaluateExtractionDraft(document,processorResult,policy) with draft fields, source spans/pages/confidence, warnings, eligible confirmation workflow, expiry, and reject reasons.

## Technical design

Verify processor/schema/version/hash/document match; normalize text as untrusted quoted data; map only allowlisted fields by document purpose; retain exact provenance; route candidates to their domain confirmation service, never direct tables/tools.
...
OCR confidence is not truth. Medication/vaccine/growth/development/memory values need exact caregiver confirmation and domain validation; no diagnosis or EAD-3 scoring.
...
Cover prompt injection in document, forged processor output, low/conflicting confidence, wrong page/hash, hidden text, unsupported field, duplicate extraction, urgent content and direct-write attempt; also fixed cutoff, stable ordering/digest, revocation, concurrent retry, and atomic rollback where applicable.
...
Commit exclusive paths with feat(documents): keep document extraction untrusted and draft; no remote mutation, deployment, activation, or unrelated edit.
