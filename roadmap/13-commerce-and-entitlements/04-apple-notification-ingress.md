---
id: AT-13-04
title: Verify Apple server notifications V2
...
touches:
  create:
    - src/commerce/ingress/apple-notification.ts
    - tests/commerce/ingress/apple-notification.test.ts
  modify: []
  test:
    - tests/commerce/ingress/apple-notification.test.ts
exclusive_paths:
...
commit:
  message: "feat(commerce): verify apple server notifications v2"
---

## Outcome

A server-only endpoint verifies Apple App Store Server Notification V2 signedPayload and nested signed transaction/renewal JWS, environment, bundle/app identifiers, dates, certificate chain, and inserts one inbox event.
...
A server-only endpoint verifies Apple App Store Server Notification V2 signedPayload and nested signed transaction/renewal JWS, environment, bundle/app identifiers, dates, certificate chain, and inserts one inbox event. Exact provider/environment identity, verification, normalization, ordering, projection, authorization, replay, tests, and evidence are included.
...
Export handleAppleNotification(request,deps) using a pinned official Apple App Store Server Library after installed/current-doc verification and AppleIngressConfig.

## Technical design

Bound body, parse only signedPayload, validate JWS chain/alg/certificates/online-check policy, bundle ID/app Apple ID/environment/signed date, nested signatures, notification UUID/type/subtype, store digest/private verified claims and acknowledge. Sandbox/prod remain separate.
...
Apple signed transaction evidence is provider input, not direct entitlement or model data. No receipt/JWS/token is exposed.
...
Cover invalid chain/signature/alg, expired/untrusted cert, wrong bundle/app/env, stale/future signedDate, nested mismatch, duplicate UUID, test notification and verifier outage; also malformed/oversized input, concurrency, environment separation, replay after crash, RLS, redacted errors/logs and deterministic final state.
...
Commit exclusive paths with feat(commerce): verify apple server notifications v2; no provider configuration, remote migration, checkout change, deployment or unrelated edit.
