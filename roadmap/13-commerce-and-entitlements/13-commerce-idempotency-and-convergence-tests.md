---
id: AT-13-13
title: Prove commerce convergence and safety availability
module: 13-commerce-and-entitlements
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-13-03, AT-13-04, AT-13-05, AT-13-09, AT-13-10, AT-13-11, AT-13-12]
blocks: [AT-14-19, AT-15-17]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - tests/evals/commerce.eval.ts
    - docs/verification/commerce.md
  modify: []
  test:
    - docs/verification/commerce.md
exclusive_paths:
...
commit:
  message: "test(commerce): prove commerce convergence and safety availability"
---

## Outcome

Provider fixtures, webhook attacks, ordering permutations, workflow replays, concurrent usage, RLS, and access-policy evals prove immutable event-to-entitlement convergence with zero paid-access escalation or safety paywall.
...
AT-13-03, AT-13-04, AT-13-05, AT-13-09, AT-13-10, AT-13-11, AT-13-12 plus current official Stripe, Apple, Google, store-policy, catalog, and schema documentation.
...
Provider fixtures, webhook attacks, ordering permutations, workflow replays, concurrent usage, RLS, and access-policy evals prove immutable event-to-entitlement convergence with zero paid-access escalation or safety paywall. Exact provider/environment identity, verification, normalization, ordering, projection, authorization, replay, tests, and evidence are included.
...
Fixtures include signed fake Stripe/Apple/Google events, authoritative lookup snapshots, catalog/account bindings, expected normalized/purchase/entitlement/usage/access states and provider/documentation versions; no real secrets/payment data.

## Technical design

Verify raw-body/JWS/OIDC paths with test keys/libraries, permute every lifecycle order/duplicate/reversal, inject crash points, reconcile, compare final state, exhaust concurrency and negative scope, run twice and hash report. Critical access/safety threshold 100%.
...
Assert model/client/flags cannot grant entitlement; cross-care-space purchase never transfers; urgent emergency and pediatrician recommendation remain available in Free, expired, conflict, outage and quota states.
...
Cover bad signatures, account/product/env mix, late refund/revoke, grace/pending, provider outage, duplicate/conflict, replay, last-unit race, RLS, flag bypass and PHI/secret scan; also malformed/oversized input, concurrency, environment separation, replay after crash, RLS, redacted errors/logs and deterministic final state.
...
Commit exclusive paths with test(commerce): prove commerce convergence and safety availability; no provider configuration, remote migration, checkout change, deployment or unrelated edit.
