---
id: AT-13-12
title: Separate rollout flags from entitlements
module: 13-commerce-and-entitlements
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-13-01, AT-13-08]
blocks: [AT-13-13, AT-14-08]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/commerce/access-policy.ts
    - tests/commerce/access-policy.test.ts
  modify: []
  test:
    - tests/commerce/access-policy.test.ts
exclusive_paths:
...
commit:
  message: "feat(commerce): separate rollout flags from entitlements"
---

## Outcome

A deterministic access policy requires authorization, country/clinical release, entitlement, usage, and rollout flag in explicit order; flags can disable rollout but can never grant paid access or safety authority.
...
AT-13-01, AT-13-08 plus current official Stripe, Apple, Google, store-policy, catalog, and schema documentation.
...
A deterministic access policy requires authorization, country/clinical release, entitlement, usage, and rollout flag in explicit order; flags can disable rollout but can never grant paid access or safety authority. Exact provider/environment identity, verification, normalization, ordering, projection, authorization, replay, tests, and evidence are included.
...
Export evaluateCapabilityAccess(context,capability,{entitlement,usage,flag,clinicalRelease}) and reason codes; safety capabilities are modeled as always available and never consulted through paid gate.

## Technical design

Order: authenticated/AuthorizedChildScope, hard safety availability, country/clinical package, internal entitlement/current reconciliation, usage reservation, then rollout flag. Unknown flag fails to disabled for optional feature; flag payload is not model-visible authority. Audit digests only.
...
No flag or payment bypasses emergency, diagnosis/prescription boundary, RLS, clinical approval, or tool confirmation. Paywall never blocks urgent/professional recommendation.
...
Cover flag true/no entitlement, entitlement/no flag, stale flag, provider/model claim, clinical release absent, quota exhausted, reconciliation pending and emergency/free cases; also malformed/oversized input, concurrency, environment separation, replay after crash, RLS, redacted errors/logs and deterministic final state.
...
Commit exclusive paths with feat(commerce): separate rollout flags from entitlements; no provider configuration, remote migration, checkout change, deployment or unrelated edit.
