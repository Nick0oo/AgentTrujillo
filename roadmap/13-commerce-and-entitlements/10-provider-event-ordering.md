---
id: AT-13-10
title: Resolve provider-specific event ordering
module: 13-commerce-and-entitlements
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-13-06, AT-13-07]
blocks: [AT-13-11, AT-13-13]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/commerce/event-ordering.ts
    - tests/commerce/event-ordering.test.ts
  modify: []
  test:
    - tests/commerce/event-ordering.test.ts
exclusive_paths:
...
commit:
  message: "feat(commerce): resolve provider-specific event ordering"
---

## Outcome

A deterministic ordering policy compares Stripe, Apple, and Google event identities/timestamps/transaction chains without assuming webhook arrival order and signals when authoritative reconciliation is required.
...
AT-13-06, AT-13-07 plus current official Stripe, Apple, Google, store-policy, catalog, and schema documentation.
...
A deterministic ordering policy compares Stripe, Apple, and Google event identities/timestamps/transaction chains without assuming webhook arrival order and signals when authoritative reconciliation is required. Exact provider/environment identity, verification, normalization, ordering, projection, authorization, replay, tests, and evidence are included.
...
Export compareProviderEvents, ProviderOrderingKey, OrderingDecision apply, ignore_duplicate, append_late, reconcile_required or invalid.

## Technical design

Use provider-specific immutable IDs and original transaction/purchase token chain, provider event/transaction signed times and lifecycle precedence defined from official docs; received_at is only tie/audit. Never discard a valid reversal solely because older. Unknown combinations reconcile.
...
Ordering cannot directly change entitlement or favor a provider to retain access; it supplies evidence to purchase projection/reconciliation.
...
Cover same timestamp, clock skew, duplicate different ID, late renewal then revoke/refund, Apple subtype, Google pending/hold, Stripe object/event mismatch, environment and unknown type; also malformed/oversized input, concurrency, environment separation, replay after crash, RLS, redacted errors/logs and deterministic final state.
...
Commit exclusive paths with feat(commerce): resolve provider-specific event ordering; no provider configuration, remote migration, checkout change, deployment or unrelated edit.
