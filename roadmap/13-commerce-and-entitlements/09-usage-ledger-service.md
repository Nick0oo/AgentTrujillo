---
id: AT-13-09
title: Enforce capability usage limits atomically
...
touches:
  create:
    - supabase/migrations/20260816280000_usage_ledger_hardening.sql
    - supabase/tests/041_usage_ledger.test.sql
...
commit:
  message: "feat(commerce): enforce capability usage limits atomically"
---

## Outcome

A forward migration and service atomically reserve, commit, release, and query care-space capability usage under exact entitlement/catalog windows without allowing model/client counters.
...
AT-13-08 plus current official Stripe, Apple, Google, store-policy, catalog, and schema documentation.
...
A forward migration and service atomically reserve, commit, release, and query care-space capability usage under exact entitlement/catalog windows without allowing model/client counters. Exact provider/environment identity, verification, normalization, ordering, projection, authorization, replay, tests, and evidence are included.
...
Export UsageLedgerService.authorizeAndReserve, commit, release, query with operation ID, capability, units and trusted context; add append-only ledger/reservation/current aggregate schema.

## Technical design

Transaction revalidates entitlement/limit/window, locks aggregate, idempotently reserves by domain operation, commits only on qualifying effect, expires abandoned reservations, and reconciles aggregate from ledger. Denials are safe and non-clinical. Basic/urgent paths consume no quota.
...
Usage limits never suppress urgent emergency or pediatrician recommendation and never expand child access. Model cannot choose units/capability or claim Premium.
...
Cover concurrent last unit, duplicate/conflicting reservation, effect failure, abandoned expiry, plan downgrade, period boundary/timezone, reconciliation pending, negative balance and cross-space; also malformed/oversized input, concurrency, environment separation, replay after crash, RLS, redacted errors/logs and deterministic final state.
...
Commit exclusive paths with feat(commerce): enforce capability usage limits atomically; no provider configuration, remote migration, checkout change, deployment or unrelated edit.
