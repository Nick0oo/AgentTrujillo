---
id: AT-11-03
title: Authenticate every Creciendo request
module: 11-creciendo-channel-and-streaming
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-11-02, AT-02-10]
blocks: [AT-11-04, AT-11-09]
worker:
  model: gpt-5.6-luna
  reasoning: max
clinical_risk: high
...
touches:
  create:
    - src/channels/creciendo/auth-middleware.ts
    - tests/channels/creciendo/auth-middleware.test.ts
  modify: []
  test:
    - tests/channels/creciendo/auth-middleware.test.ts
exclusive_paths:
...
commit:
  message: "feat(channel): authenticate every creciendo request"
---

## Outcome

A middleware verifies Supabase bearer JWT, issuer/audience/signature/time, request correlation, rate class, and current profile before any route or stream sends child/session data.
...
AT-11-02, AT-02-10 plus installed Eve 0.27.1 channel behavior and current Supabase session schema.
...
A middleware verifies Supabase bearer JWT, issuer/audience/signature/time, request correlation, rate class, and current profile before any route or stream sends child/session data. Exact route/event schemas, ordering, authentication, lifecycle, cancellation, replay, errors, tests, and evidence are included.
...
Export authenticateCreciendoRequest(request,deps), AuthenticatedChannelRequest, and withChannelAuth(handler); token/user are never serialized to model/events/logs.

## Technical design

Parse one Authorization bearer, verify via pinned JWKS/cache policy and module 02 authenticator, enforce clock skew/expiry, attach opaque actor/context, reject cookies/query tokens, strip auth before downstream telemetry, and rate-limit by safe opaque keys.
...
Authentication does not imply child access; urgent response still requires authenticated request in this private product channel and is never paywalled.
...
Cover missing/duplicate/malformed token, wrong issuer/audience/algorithm, expiry/not-before, key rotation/outage, revoked user, query token, log leak and rate limit; also unknown fields, oversized input, authorization revocation, concurrent requests, deterministic bytes/order, cancellation and zero unintended effect.
...
Commit exclusive paths with feat(channel): authenticate every creciendo request; no public deployment, remote mutation, or unrelated edit.
