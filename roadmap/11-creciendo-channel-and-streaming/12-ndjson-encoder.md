---
id: AT-11-12
title: Encode bounded NDJSON frames
module: 11-creciendo-channel-and-streaming
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-11-11]
blocks: [AT-11-07, AT-11-14, AT-11-16]
worker:
  model: gpt-5.6-luna
  reasoning: max
clinical_risk: high
...
touches:
  create:
    - src/channels/creciendo/ndjson-encoder.ts
    - tests/channels/creciendo/ndjson-encoder.test.ts
  modify: []
  test:
    - tests/channels/creciendo/ndjson-encoder.test.ts
exclusive_paths:
...
commit:
  message: "feat(channel): encode bounded ndjson frames"
---

## Outcome

A streaming encoder serializes each validated ChannelEvent as exactly one UTF-8 JSON line with bounded size, deterministic keys, flush/backpressure, cancellation, and no partial malformed frame.
...
A streaming encoder serializes each validated ChannelEvent as exactly one UTF-8 JSON line with bounded size, deterministic keys, flush/backpressure, cancellation, and no partial malformed frame. Exact route/event schemas, ordering, authentication, lifecycle, cancellation, replay, errors, tests, and evidence are included.
...
Export encodeNdjsonEvent, createNdjsonStream, NDJSON_CONTENT_TYPE, frame limit and encoder errors.

## Technical design

Exact-schema parse before JSON.stringify; reject newline/control ambiguity via JSON escaping, cap encoded bytes, append one LF, honor writer desiredSize/abort, emit heartbeat event not blank line, and never concatenate raw model text.
...
Encoder cannot leak unvalidated fields or append emergency explanation. Encoding failure closes stream with safe resumable state if headers allow.
...
Cover Unicode/grapheme, embedded newline, huge widget/text, circular/BigInt, partial writer failure, abort/backpressure, heartbeat and deterministic bytes; also unknown fields, oversized input, authorization revocation, concurrent requests, deterministic bytes/order, cancellation and zero unintended effect.
...
Commit exclusive paths with feat(channel): encode bounded ndjson frames; no public deployment, remote mutation, or unrelated edit.
