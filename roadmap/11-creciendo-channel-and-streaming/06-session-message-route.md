---
id: AT-11-06
title: Accept one authorized session message
module: 11-creciendo-channel-and-streaming
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-11-05, AT-04-11, AT-10-33]
blocks: [AT-11-07]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/channels/creciendo/routes/session-message.ts
    - tests/channels/creciendo/routes/session-message.test.ts
  modify: []
  test:
    - tests/channels/creciendo/routes/session-message.test.ts
exclusive_paths:
...
commit:
  message: "feat(channel): accept one authorized session message"
---

## Outcome

POST /creciendo/v1/sessions/:id/messages validates ownership, idempotency, bounded normalized content, and runs deterministic safety preflight before any message persistence or Eve invocation.
...
AT-11-05, AT-04-11, AT-10-33 plus installed Eve 0.27.1 channel behavior and current Supabase session schema.
...
POST /creciendo/v1/sessions/:id/messages validates ownership, idempotency, bounded normalized content, and runs deterministic safety preflight before any message persistence or Eve invocation. Exact route/event schemas, ordering, authentication, lifecycle, cancellation, replay, errors, tests, and evidence are included.
...
Export handleSessionMessage(request,auth,deps); input has content parts, client message ID/idempotency, continuation token and capabilities—not child/authority/tool choice.

## Technical design

Reauthorize session/child/security version, normalize message, run safety preflight synchronously. Critical result persists minimal safety evidence/event and terminates with emergency-only response; otherwise atomically append client message command and start/resume Eve turn. Images/documents are references, not raw arbitrary URLs.
...
Urgent branch invokes no LLM/tool/workflow/notification. Non-urgent model still cannot diagnose/prescribe; attachment/extraction stays draft.
...
Cover session enumeration, duplicate/conflicting message, stale cursor/token, revoked child, malformed content, safety engine unavailable, Eve start failure, cancellation race and urgent injection; also unknown fields, oversized input, authorization revocation, concurrent requests, deterministic bytes/order, cancellation and zero unintended effect.
...
Commit exclusive paths with feat(channel): accept one authorized session message; no public deployment, remote mutation, or unrelated edit.
