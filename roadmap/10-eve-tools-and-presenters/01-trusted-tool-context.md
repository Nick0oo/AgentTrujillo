---
id: AT-10-01
title: Build immutable trusted tool context
module: 10-eve-tools-and-presenters
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-01-16, AT-02-16]
blocks: [AT-10-02]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - agent/runtime/trusted-tool-context.ts
    - tests/agent/runtime/trusted-tool-context.test.ts
  modify: []
  test:
    - tests/agent/runtime/trusted-tool-context.test.ts
exclusive_paths:
...
commit:
  message: "feat(runtime): build immutable trusted tool context"
---

## Outcome

One request-only context supplies authenticated user, AuthorizedChildScope, session ownership, locale/country, entitlements, trace IDs, cancellation, and trusted service ports to every tool.
...
AT-01-16, AT-02-16 and the exact Eve 0.27.1/AI SDK/domain contracts named below.
...
One request-only context supplies authenticated user, AuthorizedChildScope, session ownership, locale/country, entitlements, trace IDs, cancellation, and trusted service ports to every tool. The leaf owns its exact schema, method, dependency injection, policy order, safe result/event, tests, and completion evidence.
...
Export TrustedToolContext, createTrustedToolContext, requireActiveChildScope, and context accessors; no field originates in model arguments.

## Technical design

Construct after JWT/session/child authorization, freeze deeply, use AsyncLocalStorage only with verified lifecycle cleanup, and separate privileged job context from guardian context.
...
No tool schema accepts authority, country, permission, entitlement, clinical package, or confirmation truth.
...
Cover forged model fields, missing/leaked async context, concurrent sessions, cancellation, revocation, job/guardian confusion and serialization; additionally unknown fields, forged authority, revoked access, cancellation, deterministic serialization, safe errors/output, and zero unintended effect.
...
Commit exclusive paths with feat(runtime): build immutable trusted tool context; no remote deployment, unrelated changes, or undocumented discovery entry.
