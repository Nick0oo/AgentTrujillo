---
id: AT-14-06
title: Protect providers with a privacy-safe circuit breaker
module: 14-model-fallback-observability-and-evals
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-14-03, AT-14-05]
blocks: [AT-14-07]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/models/provider-circuit-breaker.ts
    - tests/models/provider-circuit-breaker.test.ts
  modify: []
  test:
    - tests/models/provider-circuit-breaker.test.ts
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(models): protect providers with a privacy-safe circuit breaker"
---

## Outcome

A per-environment/provider/model/request-class circuit breaker prevents retry storms using redacted aggregate failure signals, bounded half-open probes, and explicit reset without storing clinical content.
...
AT-14-03, AT-14-05 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A per-environment/provider/model/request-class circuit breaker prevents retry storms using redacted aggregate failure signals, bounded half-open probes, and explicit reset without storing clinical content. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export ProviderCircuitBreaker.beforeAttempt, recordSuccess, recordFailure, state snapshot and policy; storage adapter supports atomic counters/window.

## Technical design

Count only classified transient provider failures, not client validation/safety refusals. Closed/open/half-open transitions use monotonic time, jittered cooldown, max probes and environment/model keys. State is operational cache, not authorization; fallback still needs release/budget.
...
Open circuit yields technical abstention or evaluated fallback, never relaxed clinical behavior. No child/guardian/session/message key in circuit data.
...
Cover concurrent threshold, rolling window, half-open race, cold start/multi-instance cache, clock, reset, auth/config failures and fallback circuit; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(models): protect providers with a privacy-safe circuit breaker; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
