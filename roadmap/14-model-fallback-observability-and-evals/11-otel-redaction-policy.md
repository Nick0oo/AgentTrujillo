---
id: AT-14-11
title: Apply OpenTelemetry redaction before export
module: 14-model-fallback-observability-and-evals
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-14-10]
blocks: [AT-14-12, AT-14-18]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/observability/otel-redaction.ts
    - tests/observability/otel-redaction.test.ts
    - package.json
    - package-lock.json
    - instrumentation.ts
  modify: []
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(observability): apply opentelemetry redaction before export"
---

## Outcome

A mandatory OpenTelemetry processor redacts/drops sensitive attributes/events/resources before console, Vercel runtime logs, OTLP/drains, or any vendor exporter receives them.
...
AT-14-10 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A mandatory OpenTelemetry processor redacts/drops sensitive attributes/events/resources before console, Vercel runtime logs, OTLP/drains, or any vendor exporter receives them. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export createRedactingSpanProcessor, redactAttributes, safeResourceAttributes and RedactionDecision; install/pin OTel packages only after current runtime compatibility verification.

## Technical design

Construct safe spans through schema, recursively drop denylisted keys/URL query/headers/body/db statement/prompt/tool/document/user data, hash approved correlation with rotating key, cap names/cardinality, sample after safety redaction, and fail closed/drop on uncertainty. Never rely on downstream vendor masking.
...
No PHI/PII, clinical content, chain-of-thought, secrets, signed URLs, provider bodies, billing tokens, or raw SQL in telemetry—even in errors/debug.
...
Cover semantic attribute aliases, nested arrays/objects, exception stack/message, baggage, links/events, exporter failure, debug flag, drain and snapshot; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(observability): apply opentelemetry redaction before export; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
