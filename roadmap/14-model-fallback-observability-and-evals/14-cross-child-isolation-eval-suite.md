---
id: AT-14-14
title: Run cross-child isolation evals
module: 14-model-fallback-observability-and-evals
status: pending
execution: parallel
parallel_group: global-evals
depends_on: [AT-14-12, AT-02-16, AT-09-19, AT-11-17]
blocks: [AT-14-19]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - tests/evals/cross-child-isolation.eval.ts
    - evals/fixtures/isolation.json
  modify: []
  test:
    - evals/fixtures/isolation.json
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "test(evals): run cross-child isolation evals"
---

## Outcome

An adversarial suite proves zero care-space, sibling, child, session, vector, document, tool, stream, workflow, Realtime, and entitlement crossover under both providers.
...
AT-14-12, AT-02-16, AT-09-19, AT-11-17 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
An adversarial suite proves zero care-space, sibling, child, session, vector, document, tool, stream, workflow, Realtime, and entitlement crossover under both providers. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Fixtures create synthetic tenants/siblings with identical names/content/vectors/IDs shaped for enumeration and expected allowed/denied resource matrix.

## Technical design

Attack every route/tool/repository/RPC/ticket/cursor/job and caches using swapped/guessed IDs, stale tokens, revoked membership, concurrent child switch and provider prompts. Verify pre-similarity scope query plans and no existence/timing leak within tolerance.
...
No model/provider can repair or override authorization. Any cross-scope datum, event, citation, count, existence or effect is critical.
...
Cover RLS/service context, session stream, memory vector, documents, anthropometry/vaccine/medication/development, workflows, billing care space and cache; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with test(evals): run cross-child isolation evals; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
