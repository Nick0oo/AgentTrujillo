...
parallel_group: null
depends_on: [AT-14-02, AT-11-14]
blocks: [AT-14-04, AT-14-06]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/models/provider-failure.ts
    - tests/models/provider-failure.test.ts
  modify: []
  test:
    - tests/models/provider-failure.test.ts
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(models): classify provider failures"
---

## Outcome

A phase-aware classifier maps Google and OpenRouter/AI SDK failures to permanent request, safety refusal, auth/billing/config, rate/capacity, timeout/network, provider-invalid, cancellation, or unknown with explicit retry/failover eligibility.
...
AT-14-02, AT-11-14 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A phase-aware classifier maps Google and OpenRouter/AI SDK failures to permanent request, safety refusal, auth/billing/config, rate/capacity, timeout/network, provider-invalid, cancellation, or unknown with explicit retry/failover eligibility. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export ProviderFailureKind, classifyProviderFailure(error,phase,provider), FailureDecision and sanitized diagnostics.

## Technical design

Normalize typed AI SDK/provider errors including OpenRouter error_type; retain HTTP/provider code only in private allowlist. Failover eligible only before any user-visible/model/tool/event effect and for transient availability classes; auth/config/input/safety/cancel/unknown do not cross providers by default.
...
A provider refusal or clinical abstention is not an outage and cannot be bypassed by fallback. No technical failure produces medical guidance.
...
Cover Google/OpenRouter error shapes, 402 credits, 429 Retry-After, 502/503, mid-stream SSE error, content filter/refusal, malformed response, abort and malicious message; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(models): classify provider failures; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
