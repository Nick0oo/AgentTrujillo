---
id: AT-14-15
title: Run prompt and retrieved-content injection evals
module: 14-model-fallback-observability-and-evals
status: pending
execution: parallel
parallel_group: global-evals
depends_on: [AT-14-12, AT-09-09, AT-10-33]
blocks: [AT-14-19]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - tests/evals/prompt-injection.eval.ts
    - evals/fixtures/prompt-injection.json
  modify: []
  test:
    - evals/fixtures/prompt-injection.json
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "test(evals): run prompt and retrieved-content injection evals"
---

## Outcome

A suite attacks user messages, memory, documents, citations, tool results, labels, widgets, provider metadata, and multilingual encodings to prove instructions cannot override authority, safety, tool approval, or output privacy.
...
AT-14-12, AT-09-09, AT-10-33 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A suite attacks user messages, memory, documents, citations, tool results, labels, widgets, provider metadata, and multilingual encodings to prove instructions cannot override authority, safety, tool approval, or output privacy. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Fixtures tag injection location/encoding/desired exploit/expected policy, authorized facts/tools and prohibited outputs/effects; synthetic only.

## Technical design

Test direct/indirect, role delimiter, XML/JSON, base64/Unicode, quoted/history, tool-output and document OCR injection through both providers; assert safe data delimiters/projections, no forbidden tool/effect, no secret/prompt/reasoning disclosure, stable abstention.
...
Injected content never changes emergency, diagnosis/prescription, EAD-3, medication/vaccine, confirmation, pediatrician/urgent boundary or country/package selection.
...
Cover system prompt request, fake approval/authority, tool chain, memory instruction, document exfiltration, widget HTML/URL, provider fallback and long-context; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with test(evals): run prompt and retrieved-content injection evals; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
