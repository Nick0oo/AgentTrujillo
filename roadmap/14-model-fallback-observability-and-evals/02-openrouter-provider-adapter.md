---
id: AT-14-02
title: Implement disabled-by-default OpenRouter adapter
...
touches:
  create:
    - src/models/openrouter-provider.ts
    - tests/models/openrouter-provider.test.ts
    - package.json
    - package-lock.json
  modify: []
  test:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(models): implement disabled-by-default openrouter adapter"
---

## Outcome

A Vercel AI SDK adapter uses @openrouter/ai-sdk-provider only for an exact evaluated fallback release, with server key, allowlisted model/routing, ZDR/data-collection policy, required parameters, timeouts, abort, and safe metadata.
...
AT-14-01 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A Vercel AI SDK adapter uses @openrouter/ai-sdk-provider only for an exact evaluated fallback release, with server key, allowlisted model/routing, ZDR/data-collection policy, required parameters, timeouts, abort, and safe metadata. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export createOpenRouterFallback(config,release) and OpenRouterAttemptMetadata; install/pin @openrouter/ai-sdk-provider in package files after current docs compatibility verification.

## Technical design

createOpenRouter receives key server-side; exact provider/model and routing constraints from release, allow_fallbacks false unless the parity artifact explicitly enumerates identical endpoints, require_parameters true, data_collection deny and ZDR required where evaluated. Strip provider metadata/errors before output/logs.
...
No arbitrary OpenRouter auto/model/provider routing, direct tool authority, PHI persistence opt-in, or fallback before release. It never handles urgent safety.
...
Cover missing key/release, unsupported tool/schema/stream option, no ZDR endpoint, 401/402/403/429/5xx, Retry-After, content policy, metadata leakage and abort; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(models): implement disabled-by-default openrouter adapter; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
