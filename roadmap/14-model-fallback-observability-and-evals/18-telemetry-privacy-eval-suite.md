...
touches:
  create:
    - tests/evals/telemetry-privacy.eval.ts
    - evals/fixtures/telemetry-canaries.json
  modify: []
  test:
    - evals/fixtures/telemetry-canaries.json
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "test(evals): run telemetry privacy evals"
---

## Outcome

A canary-based suite proves logs, traces, metrics, drains/exporter payloads, workflow/operator views, eval reports, errors, and audit streams contain no PHI, prompts, reasoning, secrets, signed URLs, vectors, billing tokens, or raw clinical content.
...
AT-14-11, AT-14-12 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A canary-based suite proves logs, traces, metrics, drains/exporter payloads, workflow/operator views, eval reports, errors, and audit streams contain no PHI, prompts, reasoning, secrets, signed URLs, vectors, billing tokens, or raw clinical content. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Fixtures inject unique synthetic canaries into every sensitive source/field and define allowed opaque digests/categories; capture in-memory exporters/console/audit/report artifacts.

## Technical design

Exercise success/failure/debug/retry/cancel across routes/providers/tools/workflows/commerce/documents; recursively scan raw and encoded forms, attribute names, exception stacks and high-cardinality identifiers; fail on any canary outside authorized synthetic fixture.
...
Clinical/safety observability remains categorical and cannot expose symptom text or enable hidden clinician review.
...
Cover nested/base64/URL encoded canaries, malicious provider/SQL errors, debug flags, drain signing, workflow args/results, source maps, eval failure and exporter outage; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with test(evals): run telemetry privacy evals; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
