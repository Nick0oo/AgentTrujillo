---
id: AT-14-12
title: Harden Eve agent-run visibility and retention
module: 14-model-fallback-observability-and-evals
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-14-10, AT-14-11, AT-01-15]
blocks: [AT-14-13, AT-14-14, AT-14-15, AT-14-16, AT-14-17, AT-14-18]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - docs/security/agent-runs-privacy.md
    - tests/security/agent-runs-privacy.test.ts
  modify: []
  test:
    - tests/security/agent-runs-privacy.test.ts
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(observability): harden eve agent-run visibility and retention"
---

## Outcome

A documented and executable policy determines what Eve/Vercel agent-run tooling stores/displays/retains, who can access it, and disables/redacts prompt, reasoning, tool payload, child data, and private URLs before production.
...
AT-14-10, AT-14-11, AT-01-15 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A documented and executable policy determines what Eve/Vercel agent-run tooling stores/displays/retains, who can access it, and disables/redacts prompt, reasoning, tool payload, child data, and private URLs before production. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Policy inventory covers Eve run/session persistence, runtime logs, Workflow dashboard/streams, AI SDK/provider metadata, Vercel logs/drains/traces, local dev, eval artifacts and operator roles.

## Technical design

Inspect actual installed/configured surfaces, set minimum retention/access, disable content capture where possible, insert local redaction before unavoidable surfaces, separate synthetic eval from production, audit operator access and deletion. Evidence screenshots/config hashes contain no PHI.
...
No Dr. Trujillo clinical access via observability, hidden case queue, or claim of review. Production debugging uses opaque trace and authorized database access only.
...
Cover default SDK logging, debug mode, workflow args/results, tool traces, stream namespaces, eval failures, source maps, operator offboarding and deletion; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(observability): harden eve agent-run visibility and retention; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
