---
id: AT-14-09
title: Return safe provider unavailability
module: 14-model-fallback-observability-and-evals
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-14-08, AT-04-10]
blocks: [AT-14-13, AT-14-17]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/models/provider-abstention.ts
    - tests/models/provider-abstention.test.ts
  modify: []
  test:
    - tests/models/provider-abstention.test.ts
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(models): return safe provider unavailability"
---

## Outcome

A final path returns a localized technical-unavailability result when Gemini and any released eligible fallback cannot start, preserving committed facts and never fabricating clinical content.
...
AT-14-08, AT-04-10 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A final path returns a localized technical-unavailability result when Gemini and any released eligible fallback cannot start, preserving committed facts and never fabricating clinical content. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export createProviderAbstention(context,failureSummary), ProviderAbstentionEvent and approved copy registry; channel error/retry metadata is effect-aware.

## Technical design

If emergency preflight critical, emergency copy wins without provider. Otherwise after eligible attempts fail pre-commit, emit approved technical unavailable text, safe retry-later only when appropriate, cursor/attempt ID, no provider details. Existing deterministic tool result may be rendered only if already committed and safe.
...
No diagnosis, treatment, clinician alert, booking, fallback-generated guess, or false claim that Dr. Trujillo reviewed. Pediatrician recommendation appears only from clinical policy, not generic outage.
...
Cover no fallback release, both circuits open, auth/config/credits, content refusal, partial tool effect, locale, entitlement and emergency coexistence; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(models): return safe provider unavailability; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
