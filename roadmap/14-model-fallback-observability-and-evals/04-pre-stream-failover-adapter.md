---
id: AT-14-04
title: Fail over only before stream commitment
module: 14-model-fallback-observability-and-evals
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-14-03, AT-13-12]
blocks: [AT-14-05]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/models/pre-stream-failover.ts
    - tests/models/pre-stream-failover.test.ts
  modify: []
  test:
    - tests/models/pre-stream-failover.test.ts
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(models): fail over only before stream commitment"
---

## Outcome

A routing adapter attempts direct Gemini first and may invoke one released OpenRouter fallback only before any token, tool call, widget, persisted assistant event, or other effect is committed.
...
AT-14-03, AT-13-12 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A routing adapter attempts direct Gemini first and may invoke one released OpenRouter fallback only before any token, tool call, widget, persisted assistant event, or other effect is committed. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export executeWithPreStreamFailover(request,context,providers), AttemptLedger and commitBarrier; caller receives one stream/result source.

## Technical design

Run authorization/safety/tool policy once, reserve request budget, start Gemini behind commit buffer until valid first safe event; on eligible pre-commit failure verify fallback release/circuit/budget and retry same canonical model messages/tools exactly once. Record attempt metadata/digests. Commit barrier chooses one source permanently.
...
Fallback cannot rerun safety/tools/effects, relax schemas, change prompt/policy, or activate through model/flag alone. Urgent never enters adapter.
...
Cover failure before/after provider response, buffered tool call, timeout, fallback unavailable, cancellation, budget, context drift, duplicate attempt and commit race; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(models): fail over only before stream commitment; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
