---
id: AT-14-17
title: Run provider/channel streaming recovery evals
module: 14-model-fallback-observability-and-evals
status: pending
execution: parallel
parallel_group: global-evals
depends_on: [AT-14-05, AT-14-09, AT-11-17]
blocks: [AT-14-19]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - tests/evals/streaming-recovery.eval.ts
    - evals/fixtures/streaming-recovery.json
  modify: []
  test:
    - evals/fixtures/streaming-recovery.json
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "test(evals): run provider/channel streaming recovery evals"
---

## Outcome

A fault-injection suite proves pre-stream-only failover, no mid-stream replay, ordered resumable NDJSON, truthful tool effects, cancellation/revocation, and terminal emergency behavior.
...
AT-14-05, AT-14-09, AT-11-17 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A fault-injection suite proves pre-stream-only failover, no mid-stream replay, ordered resumable NDJSON, truthful tool effects, cancellation/revocation, and terminal emergency behavior. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Fixture matrix injects Google/OpenRouter/channel/database failures before first event, after each event/tool phase, disconnect/backpressure, cursor reconnect and revocation with expected attempts/events/effects.

## Technical design

Use fake providers conforming to AI SDK plus released-candidate integration where authorized; capture raw frames/persisted sequence/attempt ledger/domain rows, reconnect, and assert one provider source after commit, no duplicate effects or post-terminal data.
...
No guessed continuation, second-provider replay, clinical fallback text, reasoning leak, or emergency append after failure.
...
Cover pre-token, buffered token, tool call/approval/commit, provider SSE error, client disconnect, server restart, cancel, revoked scope and both providers down; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with test(evals): run provider/channel streaming recovery evals; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
