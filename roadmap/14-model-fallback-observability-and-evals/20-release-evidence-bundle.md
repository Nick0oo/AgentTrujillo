---
id: AT-14-20
title: Assemble signed release evidence bundle
module: 14-model-fallback-observability-and-evals
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-14-19, AT-08-17, AT-09-19, AT-10-33, AT-12-18, AT-13-13]
blocks: [AT-15-04, AT-15-21]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
touches:
  create:
    - src/release/evidence-bundle.ts
    - tests/release/evidence-bundle.test.ts
  modify: []
  test:
    - tests/release/evidence-bundle.test.ts
exclusive_paths:
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(release): assemble signed release evidence bundle"
---

## Outcome

A reproducible builder assembles checksummed manifests for code, migrations, Eve discovery, models/providers, prompts/tools/widgets, clinical packages/approvals, eval reports, SBOM/scans, observability/privacy, workflows, commerce and known blockers into one immutable release candidate.
...
AT-14-19, AT-08-17, AT-09-19, AT-10-33, AT-12-18, AT-13-13 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A reproducible builder assembles checksummed manifests for code, migrations, Eve discovery, models/providers, prompts/tools/widgets, clinical packages/approvals, eval reports, SBOM/scans, observability/privacy, workflows, commerce and known blockers into one immutable release candidate. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export buildReleaseEvidence(input), verifyReleaseEvidence(bundle), ReleaseEvidenceManifest and signature/approval slots; bundle contains references/digests, not PHI or secrets.

## Technical design

Require clean commit, exact dependency lock/runtime, environment target, source/package approvals, JUnit/JSON hashes and zero critical results; canonicalize/sign engineering bundle, add Dr. clinical attestation references, store privately, verify byte-for-byte. Missing/expired artifact blocks release.
...
Evidence cannot waive clinical, security, legal, provider, migration, backup, rollback or country gates and never claims Dr. review beyond signed artifacts.
...
Cover dirty tree, stale/missing report, digest tamper, wrong environment/model/package, expired approval, secret/PHI scan, nondeterministic build and signature verification; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(release): assemble signed release evidence bundle; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
