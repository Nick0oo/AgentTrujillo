...
parallel_group: null
depends_on: [AT-01-17, AT-10-33, AT-13-12]
blocks: [AT-14-02]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(models): define model routing policy"
---

## Outcome

Closed server-only contracts define Gemini direct primary, disabled-by-default OpenRouter fallback candidates, capability/parity releases, request classes, budgets, failover eligibility, abstention, and provider identity.
...
AT-01-17, AT-10-33, AT-13-12 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
Closed server-only contracts define Gemini direct primary, disabled-by-default OpenRouter fallback candidates, capability/parity releases, request classes, budgets, failover eligibility, abstention, and provider identity. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export ModelPolicy, PrimaryModelIdentity, FallbackRelease, RequestClass, ProviderAttempt, FailoverDecision and resolveModelPolicy; model/client cannot supply provider/model.

## Technical design

Pin primary Google API model/config from module 01. Fallback release requires exact OpenRouter model/provider routing/options, tool/structured/stream capabilities, effective date, evidence digest and approval. No release means disabled. Safety preflight never uses a model.
...
Model policy cannot bypass deterministic engines, authority, tools, entitlements, country/clinical packages, or emergency output. Cheapest/available model is not automatically selected.
...
Cover unknown request class, missing capability, release overlap/revocation, client model hint, environment mix, context/token budget and policy digest; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(models): define model routing policy; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
