---
id: AT-14-10
title: Define privacy-minimal observability schema
module: 14-model-fallback-observability-and-evals
status: pending
execution: sequential
parallel_group: null
depends_on: [AT-12-18, AT-10-05]
blocks: [AT-14-11, AT-14-12]
worker:
  model: gpt-5.6-luna
  reasoning: max
...
forbidden_paths:
  ...
  - supabase/migrations/**
commit:
  message: "feat(observability): define privacy-minimal observability schema"
---

## Outcome

A closed versioned schema defines structured logs, metrics, and traces for routes, providers, tools, workflows, commerce, streams, and safety controls using opaque correlation and bounded categorical dimensions only.
...
AT-12-18, AT-10-05 plus current Google, OpenRouter, AI SDK, Eve, Vercel observability and eval documentation.
...
A closed versioned schema defines structured logs, metrics, and traces for routes, providers, tools, workflows, commerce, streams, and safety controls using opaque correlation and bounded categorical dimensions only. Exact types, provider/config identities, policy order, privacy, thresholds, adversarial tests, artifacts, and completion evidence are included.
...
Export ObservabilityEvent union, emitOperationalEvent, schemas, cardinality budget and sensitivity scanner.

## Technical design

Allow timestamp/environment/release/route/tool/provider-model policy IDs, outcome/error class, durations/token/cost buckets, retry/effect flags, opaque trace/session hash; forbid user/child/care-space IDs reversible to records, content, notes, URLs/tokens, vectors, prompts/reasoning/raw errors. Unknown events reject.
...
Safety telemetry records rule/package/result category and latency only, never symptoms/message/copy. Observability cannot become clinical memory or authorization.
...
Cover nested PHI/secrets, high-cardinality IDs, malicious errors, unknown fields/version, oversized attributes, sink failure, clock and sampling; also configuration/version drift, cancellation, provider outage, deterministic serialization, privacy scan, and zero critical bypass.
...
Commit exclusive paths with feat(observability): define privacy-minimal observability schema; no OpenRouter activation, observability vendor mutation, deployment, remote state change or unrelated edit.
